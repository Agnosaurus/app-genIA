Here is the code of a class, used to imgest data in a previous version of this app

Please adapt this code to fit the actual implementation, should look obvious. Mostly package changes.
This new PublicationImportService should be in the eu.askadev.magic.service package , should be named PublicationImportService

"import eu.askadev.magic.entity.Publication;
    ...
    public void importCsv(MultipartFile file) throws Exception {
        ...
        try (BOMInputStream bomInputStream = new BOMInputStream(file.getInputStream());
            ...
            for (CSVRecord record : csvParser) {
                ...
                publication.setNotes(Set.of(record.get("Notes").split(";")));
                publication.setFileAttachments(Set.of(record.get("File Attachments").split(";")));
                publication.setLinkAttachments(Set.of(record.get("Link Attachments").split(";")));
                publication.setManualTags(Set.of(record.get("Manual Tags").split(";")));
                publication.setAutomaticTags(Set.of(record.get("Automatic Tags").split(";")));
